import random
import streamlit as st
import streamlit.components.v1 as components

# 1. 페이지 기본 설정
st.set_page_config(
    page_title="오늘의 운세 복권 🎲",
    page_icon="🔮",
    layout="centered"
)

# 2. 커스텀 CSS
st.markdown("""
    <style>
    .stApp {
        background: linear-gradient(180deg, #F8FAFC 0%, #EDF2F7 100%);
    }
    .title-container {
        text-align: center;
        padding: 20px 0 10px 0;
    }
    .main-title {
        font-size: 2.5rem;
        font-weight: 800;
        color: #1A202C;
        margin-bottom: 6px;
        letter-spacing: -0.5px;
    }
    .sub-title {
        font-size: 1rem;
        color: #4A5568;
        font-weight: 500;
    }
    .intro-card {
        background: white;
        border-radius: 20px;
        padding: 25px;
        text-align: center;
        box-shadow: 0 10px 20px -5px rgba(0, 0, 0, 0.05);
        border: 1px solid #E2E8F0;
        margin-bottom: 20px;
    }
    .intro-badge {
        display: inline-block;
        background-color: #EDF2F7;
        color: #2D3748;
        font-weight: 700;
        padding: 5px 14px;
        border-radius: 20px;
        font-size: 0.85rem;
        margin-bottom: 10px;
    }
    </style>
""", unsafe_allow_html=True)

# 3. 세션 상태 초기화
if "drawn" not in st.session_state:
    st.session_state.drawn = False
if "count" not in st.session_state:
    st.session_state.count = 0

MAX_DRAWS = 3 # 하루 최대 3회 제한

# 4. 타이틀 영역
st.markdown("""
    <div class='title-container'>
        <div class='main-title'>🎲 오늘의 운세 복권</div>
        <div class='sub-title'>마우스로 복권을 직접 긁어 오늘의 운세를 확인해보세요!</div>
    </div>
""", unsafe_allow_html=True)

st.write("")

# ----------------------------------------------------
# 화면 1: 카테고리 선택 화면
# ----------------------------------------------------
if not st.session_state.drawn:
    st.markdown(f"""
        <div class='intro-card'>
            <div class='intro-badge'>🔮 TODAY's FORTUNE</div>
            <h3 style='margin-top:0; color:#1A202C; font-size:1.25rem;'>궁금한 운세를 선택하고 복권을 뽑아보세요</h3>
            <p style='color:#718096; font-size:0.9rem; margin-bottom:0;'>
                <b>(오늘의 남은 기회: {MAX_DRAWS - st.session_state.count} / {MAX_DRAWS}회)</b>
            </p>
        </div>
    """, unsafe_allow_html=True)

    category = st.selectbox(
        "👉 뽑고 싶은 운세를 선택해주세요",
        ["종합운 🌟", "재물운 💎", "학업운 📚", "금전운 💰", "연애운 💕"]
    )

    st.write("")

    if st.session_state.count >= MAX_DRAWS:
        st.warning("⚠️ 오늘의 뽑기 기회를 모두 사용하셨습니다! 내일 다시 시도해주세요. 🍀")
        st.button("🎲 운세 복권 뽑기", disabled=True, use_container_width=True)
    else:
        if st.button("🎲 운세 복권 뽑기!", use_container_width=True, type="primary"):
            st.session_state.selected_category = category
            st.session_state.count += 1
            st.session_state.drawn = True
            st.rerun()

# ----------------------------------------------------
# 화면 2: 긁기 화면 (카테고리별 맞춤 운세 데이터)
# ----------------------------------------------------
else:
    # 카테고리별 특화 운세 데이터 세트
    category_fortunes = {
        "종합운": {
            "대길 (大吉)": {
                "stars": "⭐⭐⭐⭐⭐", "emoji": "🥳",
                "summary": "뭘 해도 이상하게 잘 풀리는 날입니다! 망설이던 일이 있다면 일단 지르고 보세요. 의외의 대박이 기다립니다.",
                "dos": "평소 원했던 일에 자신 있게 도전해보기!", "donts": "다 잘 되고 있는데 사서 걱정하기."
            },
            "길 (吉)": {
                "stars": "⭐⭐⭐⭐☆", "emoji": "😎",
                "summary": "전반적으로 기분 좋은 일들이 소소하게 터지는 날입니다. 보람찬 하루를 보낼 수 있는 매끄러운 흐름이에요.",
                "dos": "맛있는 디저트로 스스로에게 소소한 포상 주기.", "donts": "분위기에 휩쓸려 싫은 일 억지로 하기."
            },
            "소길 (小吉)": {
                "stars": "⭐⭐⭐☆☆", "emoji": "🙂",
                "summary": "크게 특별할 건 없지만 평화롭고 잔잔한 하루입니다. 무탈한 일상의 소확행을 만끽하세요.",
                "dos": "최애 음악 들으며 편안하게 휴식하기.", "donts": "남들 SNS 보며 나와 비교하고 우울해하기."
            },
            "평 (平)": {
                "stars": "⭐⭐☆☆☆", "emoji": "😐",
                "summary": "잔잔한 호수 같은 날입니다. 큰 변화보다는 기존의 상태를 잘 유지하며 평정심을 지키는 게 좋습니다.",
                "dos": "퇴근/하교 후 좋아하는 영상 보며 푹 쉬기.", "donts": "잘 모르는 일에 충동적으로 결정 내리기."
            },
            "흉 (凶)": {
                "stars": "⭐☆☆☆☆", "emoji": "🫠",
                "summary": "사소한 해프닝이나 억울한 일이 생길 수 있습니다. '오늘 액땜했다!' 치고 넘기는 대범함이 필요해요.",
                "dos": "한 귀로 듣고 한 귀로 흘리는 무던함 갖기.", "donts": "홧김에 쏘아붙였다가 일 크게 키우기."
            }
        },
        "재물운": {
            "대길 (大吉)": {
                "stars": "⭐⭐⭐⭐⭐", "emoji": "💎",
                "summary": "생각지도 못한 곳에서 눈부신 자산 가치가 올라가는 날입니다! 투자나 가치 있는 물건을 알아보기에 최적입니다.",
                "dos": "장기적인 자산 계획 세우고 유용한 정보 스크랩하기.", "donts": "일확천금 노리고 검증 안 된 곳에 올인하기."
            },
            "길 (吉)": {
                "stars": "⭐⭐⭐⭐☆", "emoji": "✨",
                "summary": "내가 노력한 만큼 알차게 결실이 맺히는 날입니다. 소소한 보너스나 기분 좋은 보상이 올 수 있어요.",
                "dos": "내 가치를 높일 수 있는 곳에 투자하기.", "donts": "돈 좀 들어왔다고 입이 가벼워져 자랑하기."
            },
            "소길 (小吉)": {
                "stars": "⭐⭐⭐☆☆", "emoji": "🪙",
                "summary": "큰 변동은 없지만 통장 잔고가 안정적으로 유지됩니다. 작은 아낌이 나중에 큰 밑천이 됩니다.",
                "dos": "지출 내역 점검하고 안 쓰는 구독 서비스 해지하기.", "donts": "'이 정도쯤이야' 하고 소소하게 낭비하기."
            },
            "평 (平)": {
                "stars": "⭐⭐☆☆☆", "emoji": "⚖️",
                "summary": "들어오는 돈과 나가는 돈이 비등비등한 날입니다. 현상 유지에 집중하는 것이 재물 지키는 길입니다.",
                "dos": "가계부 정리하며 수입/지출 균형 맞추기.", "donts": "남의 말만 믿고 섣부른 재테크 시도하기."
            },
            "흉 (凶)": {
                "stars": "⭐☆☆☆☆", "emoji": "📉",
                "summary": "갑작스러운 지출이나 손실 위험이 있습니다. 귀중품이나 자산 관리에 각별히 신경 써야 하는 날입니다.",
                "dos": "지갑 닫고 지출 최소화하는 '지출 제로 데이' 도전하기.", "donts": "보증을 서거나 큰 액수의 거래 진행하기."
            }
        },
        "학업운": {
            "대길 (大吉)": {
                "stars": "⭐⭐⭐⭐⭐", "emoji": "🎓",
                "summary": "두뇌 회전이 엄청나게 빠른 날입니다! 그동안 안 풀리던 문제나 난해한 개념이 한눈에 파악됩니다.",
                "dos": "어려운 과목이나 미뤄둔 과제 한 번에 파고들기.", "donts": "공부 잘된다고 과만 믿고 밤샘하다 체력 무너뜨리기."
            },
            "길 (吉)": {
                "stars": "⭐⭐⭐⭐☆", "emoji": "📝",
                "summary": "집중력이 쑥쑥 올라가는 날입니다. 단시간 몰입해서 공부하면 기대 이상의 효율이 나는 날이에요.",
                "dos": "오늘의 공부 목표 세우고 하나씩 스티커 붙여가며 완수하기.", "donts": "스마트폰 알림 켜두고 중간중간 딴짓하기."
            },
            "소길 (小吉)": {
                "stars": "⭐⭐⭐☆☆", "emoji": "📖",
                "summary": "무난하게 계획한 만큼 학습량을 채울 수 있는 날입니다. 차근차근 복습하는 데 집중하면 좋습니다.",
                "dos": "핵심 내용 위주로 요약 노트 만들어보기.", "donts": "남들과 공부 분량 비교하며 조급해하기."
            },
            "평 (平)": {
                "stars": "⭐⭐☆☆☆", "emoji": "🥱",
                "summary": "조금 지루함이 느껴지거나 잡생각이 들 수 있습니다. 억지로 잡고 있기보다 짧고 굵게 집중하세요.",
                "dos": "50분 공부 후 10분 스트레칭 꼭 지키기.", "donts": "졸리는데 억지로 책상에 앉아서 멍때리기."
            },
            "흉 (凶)": {
                "stars": "⭐☆☆☆☆", "emoji": "🤯",
                "summary": "집중이 잘 안 되고 자꾸만 딴길로 새기 쉬운 날입니다. 쉬운 것부터 차근차근 푸는 게 정신건강에 좋습니다.",
                "dos": "책상 위 깨끗하게 정돈하고 단어 몇 개라도 제대로 외우기.", "donts": "벼락치기 하겠다고 과도한 목표 설정하기."
            }
        },
        "금전운": {
            "대길 (大吉)": {
                "stars": "⭐⭐⭐⭐⭐", "emoji": "💰",
                "summary": "지갑에 꽁돈이 생기거나 뜻밖의 할인, 포인트 적립 등 확실한 금전적 혜택이 찾아오는 기분 좋은 날입니다!",
                "dos": "이벤트 응모나 쿠폰 혜택 야무지게 챙기기.", "donts": "공돈 생겼다고 바로 유흥비로 탕진하기."
            },
            "길 (吉)": {
                "stars": "⭐⭐⭐⭐☆", "emoji": "💳",
                "summary": "합리적인 소비를 할 수 있는 날입니다. 사고 싶었던 물건을 최저가나 좋은 조건으로 득템할 기회가 생깁니다.",
                "dos": "필요했던 물건 시세 비교해보고 똑똑하게 구매하기.", "donts": "스트레스 해소용 충동 쇼핑하기."
            },
            "소길 (小吉)": {
                "stars": "⭐⭐⭐☆☆", "emoji": "🛍️",
                "summary": "쓸 데는 쓰고 지킬 때는 지키는 수지타산이 딱 맞는 날입니다. 예산 안에서 알뜰하게 활용하세요.",
                "dos": "오늘 사용할 생활비 한도 미리 정해두기.", "donts": "카드 할부로 얼렁뚱땅 결제해버리기."
            },
            "평 (平)": {
                "stars": "⭐⭐☆☆☆", "emoji": "💸",
                "summary": "생각지 못한 소소한 지출(교통비, 경조사비 등)이 생길 수 있습니다. 예비비를 약간 준비해두세요.",
                "dos": "지출 전 '진짜 필요한가?' 3번 생각해보기.", "donts": "지갑 사정 고려 안 하고 턱턱 쏘기."
            },
            "흉 (凶)": {
                "stars": "⭐☆☆☆☆", "emoji": "💸",
                "summary": "지갑이 홀쭉해지기 쉬운 날입니다. 과소비나 스팸/사기성 문자에 속지 않도록 각별히 주의해야 합니다.",
                "dos": "외출 시 필요한 카드 한 장만 챙기기.", "donts": "출처 불분명한 링크 클릭하거나 얼김에 송금하기."
            }
        },
        "연애운": {
            "대길 (大吉)": {
                "stars": "⭐⭐⭐⭐⭐", "emoji": "💕",
                "summary": "매력이 대폭발하는 날입니다! 솔로는 매력적인 인연을 만나고, 커플은 사랑이 더욱 깊어집니다.",
                "dos": "마음에 드는 사람에게 먼저 미소 지으며 인사하기.", "donts": "밀당한답시고 너무 냉정하게 굴기."
            },
            "길 (吉)": {
                "stars": "⭐⭐⭐⭐☆", "emoji": "💖",
                "summary": "대화가 부드럽게 잘 통하고 호감이 상승하는 날입니다. 따뜻한 한마디가 연애운을 확 끌어올려 줍니다.",
                "dos": "상대방의 작은 변화 칭찬해주기 (헤어, 옷 등).", "donts": "과거 이야기 꺼내서 분위기 어색하게 만들기."
            },
            "소길 (小吉)": {
                "stars": "⭐⭐⭐☆☆", "emoji": "💌",
                "summary": "소소하고 설레는 감정이 싹트는 날입니다. 평소 친했던 사람과의 관계가 새롭게 보일 수 있어요.",
                "dos": "가벼운 안부 메시지로 마음 전해보기.", "donts": "너무 진지하거나 부담스러운 고백 지르기."
            },
            "평 (平)": {
                "stars": "⭐⭐☆☆☆", "emoji": "🌿",
                "summary": "편안하고 잔잔한 감정이 유지되는 날입니다. 자극적인 이벤트보다는 나 자신의 매력을 다지는 게 좋습니다.",
                "dos": "자신감을 채워주는 예쁜 옷차림 신경 쓰기.", "donts": "상대방의 행동 하나하나에 과하게 의미 부여하기."
            },
            "흉 (凶)": {
                "stars": "⭐☆☆☆☆", "emoji": "💔",
                "summary": "사소한 오해로 서운함이 생기거나 말다툼이 일어날 수 있습니다. 한 템포 쉬어가는 지혜가 필요합니다.",
                "dos": "서운한 점이 있어도 감정 가라앉고 대화하기.", "donts": "자존심 세우며 꼬투리 잡고 비꼬기."
            }
        }
    }

    # 다양하게 확장된 색상표
    lucky_colors = [
        "로즈 핑크 🌸", "포레스트 그린 🌲", "미드나잇 블루 🌙", "버터 옐로우 🧈", "아이보리 화이트 🤍",
        "라벤더 퍼플 💜", "피치 코랄 🍑", "스카이 블루 ☁️", "민트 그린 🌿", "선셋 오렌지 🌅",
        "크림 베이지 🍦", "에메랄드 그린 💚", "와인 레드 🍷", "네이비 🌌", "올리브 그린 🫒",
        "딥 바이올렛 🔮", "펄 실버 🩶", "클래식 골드 💛", "차콜 그레이 🖤", "오션 틸 🌊"
    ]
    
    lucky_places = ["햇살 잘 드는 카페 ☕", "아늑한 내 방 침대 🛌", "조용한 공원 산책로 🌿", "자주 가는 편의점 🏪", "서점이나 소품샵 📚"]
    
    music_list = [
        "🎧 신나는 시티팝 (드라이브하는 기분 내기)",
        "🎧 잔잔한 로파이(Lo-Fi) 비트 (힐링이 필요할 때)",
        "🎧 청량한 밴드 사운드 (텐션 올리고 싶을 때)",
        "🎧 따뜻한 아쿠스틱 기타 곡 (마음 정리가 필요할 때)",
        "🎧 신나는 K-POP 플레이리스트 (에너지 충전!)"
    ]
    healing_quotes = [
        "“오늘 하루 정도는 대충 살아도 세상은 잘 돌아갑니다.”",
        "“뜻밖의 행운은 원래 기대 안 하고 있을 때 툭 찾아오는 법이에요.”",
        "“어차피 지나갈 하루라면, 최대한 웃으면서 즐겁게 보내버립시다!”",
        "“남들 시선 상관없이 오늘 내가 제일 기분 좋은 게 정답입니다.”",
        "“오늘 어떤 일이 있든, 결국엔 전부 다 잘 될 거니까 걱정 마세요.”"
    ]

    # 선택된 카테고리 명칭 추출 (예: "재물운 💎" -> "재물운")
    raw_cat = st.session_state.selected_category
    cat_name = raw_cat.split()[0]

    # 해당 카테고리의 운세 데이터 셋 가져오기
    current_category_data = category_fortunes.get(cat_name, category_fortunes["종합운"])

    # 무작위 상태 뽑기
    status_key = random.choice(list(current_category_data.keys()))
    res = current_category_data[status_key]

    # 행운 요소 및 부가 데이터 추출
    color = random.choice(lucky_colors)
    place = random.choice(lucky_places)
    number = random.randint(1, 99)
    bgm = random.choice(music_list)
    quote = random.choice(healing_quotes)

    # HTML/JS 자동 긁기 기능
    scratch_html = f"""
    <!DOCTYPE html>
    <html>
    <head>
        <style>
            body {{
                font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
                display: flex;
                flex-direction: column;
                align-items: center;
                justify-content: center;
                margin: 0;
                padding: 10px;
                background: transparent;
            }}
            .scratch-container {{
                position: relative;
                width: 100%;
                max-width: 480px;
                border-radius: 20px;
                box-shadow: 0 10px 25px rgba(0,0,0,0.1);
                overflow: hidden;
                background: white;
                border: 2px solid #E2E8F0;
            }}
            .content {{
                padding: 25px;
                text-align: center;
                box-sizing: border-box;
            }}
            .emoji {{ font-size: 3.5rem; margin-bottom: 5px; }}
            .status {{ font-size: 1.8rem; font-weight: 800; color: #1A202C; margin-bottom: 5px; }}
            .stars {{ font-size: 1.2rem; color: #ECC94B; margin-bottom: 15px; }}
            .summary {{
                font-size: 0.95rem; line-height: 1.6; color: #2D3748;
                background: #F7FAFC; padding: 15px; border-radius: 12px;
                border-left: 4px solid #4A5568; text-align: left; margin-bottom: 15px;
            }}
            .box-grid {{
                display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 15px;
            }}
            .do-box {{ background: #F0FDF4; border: 1px solid #BBF7D0; padding: 10px; border-radius: 10px; font-size: 0.85rem; color: #166534; text-align: left; }}
            .dont-box {{ background: #FEF2F2; border: 1px solid #FECACA; padding: 10px; border-radius: 10px; font-size: 0.85rem; color: #991B1B; text-align: left; }}
            
            .lucky-grid {{
                display: grid;
                grid-template-columns: 1fr 1fr 1fr;
                gap: 8px;
                margin-bottom: 15px;
            }}
            .lucky-card {{
                background: #F8FAFC;
                border: 1px solid #E2E8F0;
                border-radius: 12px;
                padding: 10px 5px;
                text-align: center;
            }}
            .lucky-label {{
                font-size: 0.75rem;
                color: #718096;
                font-weight: 600;
                margin-bottom: 4px;
            }}
            .lucky-val {{
                font-size: 0.85rem;
                color: #1A202C;
                font-weight: 700;
            }}

            .music {{ background: #F0F9FF; color: #0369A1; padding: 10px; border-radius: 10px; font-size: 0.85rem; margin-bottom: 10px; font-weight: 600; }}
            .quote {{ font-size: 0.85rem; color: #4A5568; font-style: italic; }}
            
            canvas {{
                position: absolute;
                top: 0;
                left: 0;
                width: 100%;
                height: 100%;
                cursor: pointer;
                touch-action: none;
                transition: opacity 0.5s ease;
            }}
            .guide-text {{
                margin-top: 8px; font-size: 0.85rem; color: #718096; font-weight: 600;
            }}
        </style>
    </head>
    <body>
        <div class="scratch-container" id="container">
            <div class="content">
                <div class="emoji">{res['emoji']}</div>
                <div class="status">[{cat_name}] {status_key}</div>
                <div class="stars">{res['stars']}</div>
                <div class="summary">💡 <b>오늘의 한줄 요약:</b><br>{res['summary']}</div>
                <div class="box-grid">
                    <div class="do-box"><b>⭕ 해보세요</b><br>{res['dos']}</div>
                    <div class="dont-box"><b>❌ 피하세요</b><br>{res['donts']}</div>
                </div>
                
                <div class="lucky-grid">
                    <div class="lucky-card">
                        <div class="lucky-label">🎨 행운의 컬러</div>
                        <div class="lucky-val">{color}</div>
                    </div>
                    <div class="lucky-card">
                        <div class="lucky-label">📍 행운의 장소</div>
                        <div class="lucky-val">{place}</div>
                    </div>
                    <div class="lucky-card">
                        <div class="lucky-label">🔢 행운의 숫자</div>
                        <div class="lucky-val">{number}</div>
                    </div>
                </div>

                <div class="music">🎵 {bgm}</div>
                <div class="quote">💌 {quote}</div>
            </div>
            <canvas id="scratchCanvas"></canvas>
        </div>
        <div class="guide-text" id="guide">👆 마우스로 쓱쓱 긁어보세요! (조금만 긁으면 알아서 열려요)</div>

        <script>
            const canvas = document.getElementById('scratchCanvas');
            const ctx = canvas.getContext('2d');
            const container = document.getElementById('container');
            const guide = document.getElementById('guide');

            let isRevealed = false;

            function initCanvas() {{
                canvas.width = container.offsetWidth;
                canvas.height = container.offsetHeight;

                ctx.fillStyle = '#CBD5E1';
                ctx.fillRect(0, 0, canvas.width, canvas.height);

                ctx.fillStyle = '#64748B';
                ctx.font = 'bold 20px sans-serif';
                ctx.textAlign = 'center';
                ctx.fillText('🪙 여기를 긁어보세요!', canvas.width / 2, canvas.height / 2 - 10);
                
                ctx.font = '14px sans-serif';
                ctx.fillText('오늘의 운세 복권', canvas.width / 2, canvas.height / 2 + 20);
            }}

            let isDrawing = false;

            function checkScratchPercentage() {{
                if (isRevealed) return;

                const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
                const pixels = imageData.data;
                let transparentPixels = 0;

                for (let i = 3; i < pixels.length; i += 4) {{
                    if (pixels[i] === 0) {{
                        transparentPixels++;
                    }}
                }}

                const percentage = (transparentPixels / (pixels.length / 4)) * 100;

                if (percentage >= 40) {{
                    isRevealed = true;
                    canvas.style.opacity = '0';
                    setTimeout(() => {{
                        canvas.style.display = 'none';
                    }}, 500);
                    guide.innerText = "✨ 오늘의 운세가 모두 공개되었습니다!";
                }}
            }}

            function scratch(e) {{
                if (!isDrawing || isRevealed) return;
                const rect = canvas.getBoundingClientRect();
                const x = (e.clientX || e.touches[0].clientX) - rect.left;
                const y = (e.clientY || e.touches[0].clientY) - rect.top;

                ctx.globalCompositeOperation = 'destination-out';
                ctx.beginPath();
                ctx.arc(x, y, 30, 0, Math.PI * 2);
                ctx.fill();

                checkScratchPercentage();
            }}

            canvas.addEventListener('mousedown', (e) => {{ isDrawing = true; scratch(e); }});
            canvas.addEventListener('mousemove', scratch);
            canvas.addEventListener('mouseup', () => isDrawing = false);
            
            canvas.addEventListener('touchstart', (e) => {{ isDrawing = true; scratch(e); }});
            canvas.addEventListener('touchmove', scratch);
            canvas.addEventListener('touchend', () => isDrawing = false);

            window.onload = initCanvas;
        </script>
    </body>
    </html>
    """

    components.html(scratch_html, height=620)

    st.write("")

    left_draws = MAX_DRAWS - st.session_state.count
    if left_draws > 0:
        if st.button(f"🔄 다른 운세 다시 뽑기 (남은 기회: {left_draws}회)", use_container_width=True):
            st.session_state.drawn = False
            st.rerun()
    else:
        st.warning("⚠️ 오늘의 재뽑기 기회를 모두 사용하셨습니다.")
        if st.button("🏠 처음 화면으로 돌아가기", use_container_width=True):
            st.session_state.drawn = False
            st.rerun()
